...
onMounted(() => {
  email.value += '@krlx.fr'
  signalLink.value += '0wn5jh60ukb2FYwVgszEagpEPviB_QOzlOACzRkHrKjL1_6AAC9Mo60ZDZZ3loek'
})
...
<template>
  <div v-if="frontmatter.home">
    <div class="grid grid-flow-row-dense sm:auto-rows-[10rem] grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-4 p-4 max-w-[1024px] mx-auto">
      <div class="card w-32 row-span-1 sm:w-auto sm:row-span-2 hover:shadow-xl transition-all group overflow-clip relative">
          <img
      ...
      <a href="/cv" class="row-span-1">
        <div class="card hover:shadow-xl transition-all group hover:bg-gray-50 h-max">
          <div class="card-body">
            ...
            <p class="group-hover:translate-x-4 transition-all">Pour connaître mon parcours</p>
          </div>
        </div>
      </a>
      ...
      <a href="/library" class="row-span-1">
        <div class="card hover:shadow-xl transition-all group hover:bg-gray-50">
          <div class="card-body">
            ...
            <p class="group-hover:translate-x-4 transition-all">La pile à lire et quelques liens</p>
          </div>
        </div>
      </a>
      <a href="/publications" class="row-span-1">
        <div class="card hover:shadow-xl transition-all group hover:bg-gray-50">
          <div class="card-body">
            ...
            <p class="group-hover:translate-x-4 transition-all">Des accomplissements d'une ancienne vie</p>
          </div>
        </div>
      </a>
      ...
      <div class="card hover:shadow-xl transition-all group hover:bg-gray-50">
        <div class="card-body">
          ...
          <a :href="`mailto:${email}`" class="group-hover:translate-x-4 transition-all hover:font-bold">{{ email }}</a>
          <a :href="signalLink" class="group-hover:translate-x-4 transition-all hover:font-bold">Signal</a>
        </div>
      </div>
    </div>
  </div>
  <div v-else>
    <a class="btn fixed left-8 bottom-8 sm:top-8 z-20" href="/">Retour</a>
  </div>
  <div v-if="frontmatter.notelist">
    <div class="prose mx-auto p-4 my-12">
      <ul>
        <li v-for="note of notes" :key="note.url">
          <a :href="note.url">{{ note.title }} - {{ note.date.string }}</a>
        </li>
      </ul>
    </div>
  </div>
